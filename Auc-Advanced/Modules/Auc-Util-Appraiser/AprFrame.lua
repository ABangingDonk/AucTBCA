...
function private.CreateFrames()
	...
	function frame.UpdateDisplay()
		...
		if tinted then
			local nr, ng, nb = frame.SetPriceColor(itemLink, 1, curBuy, curBuy, r,g,b)
			if nr then 
				r, g, b = nr, ng, nb
				a = 0.4 
			end
		end
		AppraiserSaleboxBuyGold:SetBackdropColor(r or 0, g or 0, b or 0, a or 0)
		...
		r,g,b,a=0,0,0,0
		if tinted then
			local nr, ng, nb = frame.SetPriceColor(itemLink, 1, curBid, curBid, r,g,b)
			if nr then 
				r, g, b = nr, ng, nb
				a = 0.4 
			end
		end
		AppraiserSaleboxBidGold:SetBackdropColor(r or 0, g or 0, b or 0, a or 0)
	...
	function frame.PostAuctions(obj)
		...
		elseif postType == "batch" then
			if not IsModifierKeyDown() and GetMouseButtonClicked() ~= "RightButton" then
				SetBasicMessageDialogText(_TRANS('APPR_Interface_BatchButtonCombination') )--This button requires you to press a combination of keys when clicked.\nSee help printed in the chat frame for further details.
			...
			if not mode then
				SetBasicMessageDialogText(_TRANS('APPR_Help_BatchUnknownKeyCombo') )--Unknown key combination pressed while clicking batch post button.
				return
			end
